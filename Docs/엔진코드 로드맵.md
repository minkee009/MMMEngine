...
- Object API -> Object, ObjectPtr, ObjectManager 로 크게 3가지 개념이 존재함
  ...
  - ObjectManager : 할당되는 모든 Object를 관리하는 중앙관리자
    - 지연파괴 제공 -> 모든 Object는 Object::CreateInstance< T >()와 Object::Destroy()로 관리할 수 있음, 이 호출은 ObjectManager::CreateObject< T >()와 ObjectManager::Destroy()를 감싸는 스크립팅 API함수임
...
- Renderer들의 Draw는 D3D11 API를 직접 이용하지 않는다
  - { RenderOrder, DepthOrder, MeshID, TexID, MatID, VSID, GSID, HSID, DSID , PSID } 정보를 구조체에 담아 보냄
...
- 추가로 특정 렌더오더 ( 0 ~ 1000 : opaque, 1001 ~ 2000 : transparent ) 에서 PSO세팅을 바꾸어 Set콜도 줄인다.
