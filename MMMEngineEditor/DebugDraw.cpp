...
void XM_CALLCONV DX::Draw(
    DirectX::PrimitiveBatch<DirectX::VertexPositionColor>* batch,
    const DirectX::BoundingSphere& sphere,
    DirectX::XMMATRIX world,
    DirectX::FXMVECTOR color)
{
    const XMVECTOR origin = XMVector3TransformCoord(XMLoadFloat3(&sphere.Center), world);
    const float radius = sphere.Radius;

    // world의 회전(및 스케일)을 적용한 축 벡터
    XMVECTOR xaxis = XMVector3TransformNormal(XMVectorScale(g_XMIdentityR0, radius), world);
    XMVECTOR yaxis = XMVector3TransformNormal(XMVectorScale(g_XMIdentityR1, radius), world);
    XMVECTOR zaxis = XMVector3TransformNormal(XMVectorScale(g_XMIdentityR2, radius), world);
